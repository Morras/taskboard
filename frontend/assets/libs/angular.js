 ...
 * a string value representing the url.  In either case, the template URL is passed through {@link $sce#getTrustedResourceUrl $sce.getTrustedResourceUrl}.
 *
 *
 ...
 * $sceDelegateProvider.resourceUrlWhitelist} and {@link ng.$sceDelegateProvider#resourceUrlBlacklist $sceDelegateProvider.resourceUrlBlacklist}
 */
 ...
 * that the URLs used for sourcing Angular templates are safe.  Refer {@link ng.$sceDelegateProvider#resourceUrlWhitelist $sceDelegateProvider.resourceUrlWhitelist} and
...
function $SceDelegateProvider() {
  ...
  this.$get = ['$injector', function($injector) {
     ...
     * `$sceDelegate.trustAs`}, returns the value that had been passed to {@link ng.$sceDelegate#trustAs `$sceDelegate.trustAs`}.
     *
     * If the passed parameter is not a value that had been returned by {@link ng.$sceDelegate#trustAs `$sceDelegate.trustAs`}, returns it as-is.
     *
 ...
 * $sce.getTrusted(context, value)} rather than to the value directly.  Directives use {@link ng.$sce#parseAs $sce.parseAs} rather than `$parse` to watch attribute bindings, which performs the
 * {@link ng.$sce#getTrusted $sce.getTrusted} behind the scenes on non-constant literals.
 *
 * As an example, {@link ng.directive:ngBindHtml ngBindHtml} uses {@link ng.$sce#parseAsHtml $sce.parseAsHtml(binding expression)}.  Here's the actual code (slightly
 * simplified):
 *
 * ```
 ...
 * served document.  You can change these by setting your own custom {@link ng.$sceDelegateProvider#resourceUrlWhitelist whitelists}
 * and {@link ng.$sceDelegateProvider#resourceUrlBlacklist blacklists} for matching such URLs.
 *
...
function $SceProvider() {
  ...
  this.$get = ['$parse', '$sceDelegate', function(
     ...
     * {@link ng.$parse $parse} and is identical when the expression is a literal constant.  Otherwise, it
 ...
 * To utilize this functionality, ensure that `$sanitize` is available, for example, by including {@link ngSanitize} in your module's dependencies (not in core Angular). In order to use {@link ngSanitize}
 ...
 * {@link $sce#trustAsResourceUrl wrap them} as trusted values. Refer to Angular's {@link ng.$sce Strict Contextual Escaping}.
 *
